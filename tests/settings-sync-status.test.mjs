...
test('renderer reconciles a missed sync terminal event from persisted timestamps', async () => {
  ...
  assert.match(source, /syncStartedDuringRenderer\(snapshot\.sync, rendererStartedAt\.current\)/)
  ...
  assert.match(source, /const syncInterval = syncing && syncStage !== "assignments"/)
  assert.match(source, /bridge\.getRendererSnapshot\(\)/)
  assert.match(source, /setInterval\(\(\) => void reconcile\(\), 5_000\)/)
})
