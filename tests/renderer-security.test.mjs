...
test('free classroom renderer uses the pure filter module without Node-only imports', async () => {
  const view = await readFile(new URL('../src/views/tools/FreeClassroomView.tsx', import.meta.url), 'utf8')
  const filter = await readFile(new URL('../core/free-classroom-filter.mjs', import.meta.url), 'utf8')
  assert.match(view, /from ["']\.\.\/\.\.\/\.\.\/core\/free-classroom-filter\.mjs["']/)
  assert.match(view, /currentShanghaiDate/)
  assert.match(view, /currentAcademicWeek/)
  assert.match(view, /periodInput/)
  assert.match(view, /periodInput, setPeriodInput\] = useState\(""\)/)
  assert.match(view, /periods.*\[value\]/s)
  assert.doesNotMatch(view, /from ["'][^"']*adapters\/jwglxt\.mjs["']/)
  assert.doesNotMatch(filter, /from\s+["']node:/)
  assert.doesNotMatch(filter, /createHash|require\s*\(/)
})
