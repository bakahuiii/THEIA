...
- COS / GitHub 自动更新只对正式 Windows 安装包生效。
- 发现新版本后可以下载并重启安装。
...
- COS 是首选更新源；当 COS 返回旧元数据或检查失败时，客户端会自动回退到 GitHub Release，并沿用回退源下载安装包。这部分不会替你改学校侧数据，只处理本机安装包。
...
- AboutSettings 通过 bridge.getUpdateStatus、bridge.checkForUpdates、bridge.downloadUpdate 和 bridge.installUpdate 管理 COS / GitHub 自动更新，COS 元数据落后或不可用时自动回退 GitHub；只有进入 downloaded 状态时才允许安装；右下角通知还可通过 bridge.skipUpdateVersion 忽略当前版本。
