...
| 来源                | 主集合                                                     | 特性                                                                                                                                                 |
| ------------------- | ---------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| JWGLXT 教务         | 个人资料、学期、课程、课表、成绩、考试、已选课程、学业进度 | 默认经统一认证浏览器通道获取；启用且配置了独立教务 API 时优先用 API。已启用 API 的本次失败会保留旧值并报告错误，不在同一轮同步中静默改走浏览器通道。 |
...
应用先加载本地快照并渲染，避免网络探测阻塞首帧；之后 UI 读取认证状态、API 地址、加密凭据状态、模型状态、选课状态与本地缓存。该首轮运行时探测完成后，渲染器发送 `theia:initialization-complete` 事件。~~外观层用它决定“北化风情”应按当前节气切换哪个内置湖景~~，不把网络探测结果混入业务数据。
...
现有 `electron/model-service.mjs` 是另一条作业/摘要生成路径。它不是顾问运行时，当前也不会接收 advisor overview。`AdvisorRuntime`、最小披露 ContextBuilder、敏感域授权、严格模型回答 schema、响应引用校验与工具循环仍未实现。
...
模型服务只在 Electron 主进程中调用 OpenAI-compatible `/v1/models` 与 `/v1/chat/completions`。它读取已准备的任务内容、题目和受限长度的附件文本，不接触学校密码、Cookie、浏览器存储或认证页面。在线测试答案在写入页面前经过现有答案结构验证。模型输出是待审核结果，不是学校提交行为的授权。
