...
认证状态不能只看 URL：通过已认证页面检查。登录页、CAS 页面和教务本地登录页都必须判为未认证。一次 CAS 登录可同时建立教务系统和北化在线THEOL的共享会话，但两个校园域仍需分别打开已认证页面确认；一个域成功不代表另一个域也已连接。启动时可用保存的统一认证凭据自动填充；Cookie 不得序列化到 store。
